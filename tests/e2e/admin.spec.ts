 ...
 * 题库下线生效（F0343）、审计入库（F0365）、AI 运营台服务端评测、登录节流。
 */

const ops = staffFixture("operations");
const support = staffFixture("support");
const aiops = staffFixture("aiops");
const throttle = staffFixture("throttle");

test("运营账号登录后台 → 题库下线生效 → 审计出现记录 → 越权被 403", async ({ page }) => {
  await page.goto("/admin-login");
  await page.getByLabel("用户名").fill(ops.username);
  await page.getByLabel("密码").fill(ops.password);
  await page.getByRole("button", { name: "登录" }).click();
  await page.waitForURL(/\/admin(\?|$)/);
  await expect(page.getByText(`${ops.displayName} · 运营`)).toBeVisible();
...
test("登录页不暴露任何默认口令", async ({ page }) => {
  await page.goto("/admin-login");
  await expect(page.getByText("演示账号")).toHaveCount(0);
  const body = (await page.textContent("body")) ?? "";
  expect(body).not.toContain("Ops@123456");
  expect(body).not.toContain("Boss@123456");
  expect(body).toContain("账号由系统管理员安全配发");
});

test("客服账号无题库写权限：接口 403", async ({ request }) => {
  const login = await request.post("/api/admin/auth/login", {
    data: { username: support.username, password: support.password },
  });
...
test("后台登录节流：连续失败达到上限后返回 429，且锁定期内正确口令同样被拒", async ({ request }) => {
  const statuses: number[] = [];
  for (let attempt = 0; attempt < 5; attempt++) {
    const res = await request.post("/api/admin/auth/login", {
      data: { username: throttle.username, password: "WrongPass#000000" },
    });
  ...
  const locked = await request.post("/api/admin/auth/login", {
    data: { username: throttle.username, password: throttle.password },
  });
  expect(locked.status()).toBe(429);
});

test("AI 运营账号在 AI 运营台服务端跑评测 → 门禁通过 + 历史入库", async ({ page }) => {
  await page.goto("/admin-login");
  await page.getByLabel("用户名").fill(aiops.username);
  await page.getByLabel("密码").fill(aiops.password);
  await page.getByRole("button", { name: "登录" }).click();
  await page.waitForURL(/\/admin(\?|$)/);
  ...
  await expect(diagSection.getByText(/通过率 \d+%/).first()).toBeVisible({ timeout: 15000 });
...
test("F0009 反馈入库：用户提交后出现在后台工单", async ({ page }) => {
  ...
  expect(data.rows.some((row) => row.text.includes(text))).toBe(true);
});
